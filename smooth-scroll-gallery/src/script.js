...
for (let i = 0; i < 4; i++) {
    const material = new THREE.MeshBasicMaterial({
        map: loader.load(`images/${i}.jpg`),
    })

    const image = new THREE.Mesh(geometry, material)

    image.position.set(Math.random() + .3, i * -1.8)
...
// mouse
window.addEventListener('wheel', omMouseWheel)

let y = 0
let position = 0

function omMouseWheel(event) {
...
const tick = () =>
{
    ...
    for (const intersect of intersects) {
        ...
        gsap.to(intersect.object.rotation, {y: -.7})
